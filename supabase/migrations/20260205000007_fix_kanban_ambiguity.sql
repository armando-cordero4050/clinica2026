-- Fix: Ambiguous column reference 'id' in get_lab_kanban
-- Root cause: id in RETURNS TABLE conflicts with table columns in queries.
...
LANGUAGE plpgsql
SECURITY DEFINER
-- This fixes the ambiguity by giving priority to table columns over variables/parameters
...
BEGIN
    ...
    IF v_user_role IN ('clinic_admin', 'clinic_doctor', 'clinic_staff', 'clinic_receptionist') THEN
        ...
        ORDER BY o.created_at DESC; -- Latest first is better for Kanban usually
    ELSE
        -- Lab/Admin sees all
        RETURN QUERY
        SELECT 
