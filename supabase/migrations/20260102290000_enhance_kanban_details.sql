-- Fix: Enhance get_lab_kanban with CORRECT syntax and proper details
-- Description: v2 with fixed JOINs
...
BEGIN
    ...
    -- Function to get readable order number
    -- We assume column order_number exists, if not fallback to ID
    ...
    IF v_user_role IN ('clinic_admin', 'clinic_doctor', 'clinic_staff', 'clinic_receptionist') THEN
        ...
        SELECT 
            o.id,
            COALESCE(o.order_number, SUBSTRING(o.id::text, 1, 8)) as order_number, -- Short ID if null
...
COMMENT ON FUNCTION public.get_lab_kanban IS 'Get kanban cards with detailed info (Patient Name, Clinic Name, Order #)';
