-- Fix: Resolve ambiguous column reference in get_lab_kanban
-- Description: Explicitly qualify all column references to avoid ambiguity
...
BEGIN
    ...
    IF v_user_role IN ('clinic_admin', 'clinic_doctor', 'clinic_staff', 'clinic_receptionist') THEN
        ...
        SELECT 
            ...
            o.priority, 
            o.due_date::DATE,
            (SELECT COUNT(*) FROM schema_lab.order_items oi WHERE oi.order_id = o.id) as items_count,
            o.timers,
            (SELECT c.name FROM schema_medical.clinics c WHERE c.id = o.clinic_id) as clinic_name,
            (SELECT s.name FROM schema_lab.order_items oi2
             JOIN schema_lab.services s ON oi2.service_id = s.id 
...
COMMENT ON FUNCTION public.get_lab_kanban IS 'Get kanban orders with fully qualified column references';
