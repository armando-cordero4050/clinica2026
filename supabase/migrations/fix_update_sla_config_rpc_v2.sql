-- Drop and recreate update_sla_config with better error handling

-- 1. Drop the old function
DROP FUNCTION IF EXISTS public.update_sla_config(TEXT, INTEGER, INTEGER);

-- 2. Create new version with JSONB return type
CREATE OR REPLACE FUNCTION public.update_sla_config(
    p_stage TEXT,
    p_target_hours NUMERIC,
    p_warning_hours NUMERIC
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
