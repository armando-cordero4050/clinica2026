-- Fix: Handle NULL auth.uid() in get_lab_dashboard_stats
-- Description: Add NULL check for auth.uid()
...
COMMENT ON FUNCTION public.get_lab_dashboard_stats IS 'Get dashboard stats filtered by clinic_id with NULL checks';
