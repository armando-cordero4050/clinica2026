-- Updated RPC to determine online status based on recent login activity
-- Since auth.sessions is not populated by default in Supabase, we use last_sign_in_at
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            ...
            AND au.last_sign_in_at > (NOW() - INTERVAL '30 minutes')
            THEN TRUE
            ELSE FALSE
...
COMMENT ON FUNCTION public.get_all_users_with_sessions IS 
  'Returns all users with online status based on recent login activity (last 30 minutes)';
