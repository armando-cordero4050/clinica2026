...
BEGIN
    ...
    -- Determine role: if contact is the same as clinic, it is admin
    v_target_role := 'clinic_staff';
    ...
    IF v_user_id IS NULL THEN
        ...
        VALUES (p_email, p_name, v_target_role, NOW())
        RETURNING id INTO v_user_id;
    ELSE
        -- Update user name and role if it's the main contact
        UPDATE public.users SET name = p_name, role = v_target_role WHERE id = v_user_id;
    END IF;
