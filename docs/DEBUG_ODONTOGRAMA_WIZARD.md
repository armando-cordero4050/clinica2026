...
### 1. Mapeo de Materiales (Ambig�edad Cl�nica)
**Riesgo:** El odontograma env�a 'Corona', pero el laboratorio tiene m�ltiples tipos (Zirconio, Metal-Cer�mica, etc.).
**Decisi�n T�cnica:** El Wizard **NO** intentar� adivinar el material.
- Se mantendr� el **Paso 1 (Selecci�n de Material)** como obligatorio.
- El pre-llenado afectar� �nicamente al **Paso 2 (Items)**, inyectando el n�mero de diente y tipo de trabajo gen�rico, evitando que el usuario tenga que volver a escribir el diente.

### 2. Persistencia del Estado Cl�nico
**Riesgo:** Si el usuario cancela el Wizard, el hallazgo en el odontograma podr�a quedar inconsistente.
**Decisi�n T�cnica:** El estado 'Corona' en el odontograma representa una **Indicaci�n Cl�nica**, independiente de si la orden de laboratorio se ha creado o no.
- Si se cancela el Wizard, la corona permanece como hallazgo cl�nico, pero sin v�nculo a orden de lab.

### 3. Contexto de Datos (Integridad Referencial)
**Riesgo:** Crear �rdenes hu�rfanas sin \patient_id\ v�lido.
**Soluci�n Refinada:** El objeto de configuraci�n inyectado al Wizard ser� robusto:
\\\	ypescript
...
### 4. Sincronizaci�n de Estados (Kanban)
**Verificaci�n:** Asegurar que el RPC \create_lab_order\ asigne el estado inicial correcto (\clinic_pending\) para que aparezca visible inmediatamente en el Kanban.
